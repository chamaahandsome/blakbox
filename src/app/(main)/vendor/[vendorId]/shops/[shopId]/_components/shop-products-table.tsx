...
const ShopProductsTable: React.FC<ShopProductsTableProps> = ({
  products,
  defaultData,
}) => {
  ...
  const handleSaveProducts = async () => {
    setIsLoading(true)
    const response = await updateShopProducts(
      JSON.stringify(liveProducts),
      defaultData.id
    )
    await saveActivityLogsNotification({
      marketId: undefined,
      description: `Update shop products | ${response.name}`,
      vendorId: defaultData.vendorId,
    })
    setIsLoading(false)
    router.refresh()
  }
