...
export async function POST(req: Request) {
  const {
    ...
    vendorConnectAccId: string
    prices: { recurring: boolean; productId: string }[]
    vendorId: string
  } = await req.json()

  const origin = req.headers.get('origin')
  if (!vendorConnectAccId || !prices.length)
  ...
  if (
    ...
    !process.env.NEXT_PUBLIC_PLATFORM_MARKET_PERCENT
  ) {
  ...
  // const marketIdConnectedAccountId = await db.vendor.findUnique({
  //   where: { id: vendorId },
  //   include: { Market: true },
  // })

  const subscriptionPriceExists = prices.find((price) => price.recurring)
  // if (!marketIdConnectedAccountId?.Market.connectAccountId) {
  //   console.log('Market is not connected')
  //   return NextResponse.json({ error: 'Market account is not connected' })
  // }
  ...
  try {
    const session = await stripe.checkout.sessions.create(
      ...
      { stripeAccount: vendorConnectAccId }
    )
