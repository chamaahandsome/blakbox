...
const editorReducer = (
  ...
  switch (action.type) {
    ...
    case 'SET_SHOPPAGE_ID':
      const { shopPageId } = action.payload
      const updatedEditorStateWithShopPageId = {
        ...state.editor,
        shopPageId,
      }

      const updatedHistoryWithShopPageId = [
        ...state.history.history.slice(0, state.history.currentIndex + 1),
        { ...updatedEditorStateWithShopPageId }, // Save a copy of the updated state
      ]

      const shopPageIdState = {
        ...state,
        editor: updatedEditorStateWithShopPageId,
        history: {
          ...state.history,
          history: updatedHistoryWithShopPageId,
          currentIndex: updatedHistoryWithShopPageId.length - 1,
        },
      }
      return shopPageIdState
