...
const TicketForm = ({ getNewTicket, laneId, vendorId }: Props) => {
  ...
  useEffect(() => {
    if (vendorId) {
      const fetchData = async () => {
        const response = await getVendorTeamMembers(vendorId)
      ...
      const response = await upsertTicket(
        {
          ...
          customerId: contact || undefined,
          value: Number(values.value),  // Convert value to a number
        },
        tags
      );
