...
const SendInvitation: React.FC<SendInvitationProps> = ({ marketId }) => {
  const { toast } = useToast()
  const userDataSchema = z.object({
    email: z.string().email(),
    role: z.enum(['MARKET_ADMIN', 'VENDOR_USER', 'VENDOR_GUEST']),
  })
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    try {
      const res = await sendInvitation(values.role, values.email, marketId)
